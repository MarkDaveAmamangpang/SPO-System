    ...
    <div class="container mx-auto px-4 py-5">
                    ...
                    <li>
                        <div class="flex items-center">
                            <a href="{{ route('admin.player-accounts') }}"
                                class="ms-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ms-2 dark:text-gray-400 dark:hover:text-white">Playes
                                Lists</a>
                        </div>
                    </li>
        ...
        <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-8">
            <div class="flex items-center mb-8">
                <img class="h-24 w-24 rounded-full mr-6" src="{{ $player->getImageURL() }}" alt="Profile Picture">
                <div>
                    <h2 class="text-xl font-semibold text-gray-900 dark:text-white">{{ $player->firstname }}
                        {{ $player->lastname }}</h2>
                    <p class="text-gray-600 dark:text-gray-400">{{ $player->email }}</p>
                    <p class="text-gray-600 dark:text-gray-400">ID Number: {{ $player->idnumber }}</p>
                </div>
            </div>
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <p class="text-gray-600 dark:text-gray-400"><span class="font-medium">Middle Name:</span>
                        {{ $player->middlename }}</p>
                    <p class="text-gray-600 dark:text-gray-400"><span class="font-medium">Sex:</span> {{ $player->sex }}
                    </p>
                    <p class="text-gray-600 dark:text-gray-400"><span class="font-medium">Birthdate:</span>
                        {{ $player->birthdate ? date('Y-m-d', strtotime($player->birthdate)) : 'N/A' }}</p>
                    <p class="text-gray-600 dark:text-gray-400"><span class="font-medium">Status:</span>
                        {{ $player->isArchived() ? 'Archived' : 'Active' }}</p>
                    <p class="text-gray-600 dark:text-gray-400"><span class="font-medium">Last Login:</span>
                        {{ $player->lastlogin ? \Carbon\Carbon::parse($player->lastlogin)->format('Y-m-d') : 'N/A' }}</p>
                </div>
            </div>
                            ...
                            <div class="flex w-0 flex-1 items-center">
                                ...
                                <div class="ml-4 flex min-w-0 flex-1 gap-2">
                                    <a href="{{ $doc->getFileURL() }}"
                                        class="text-blue-600 hover:text-blue-500">{{ $doc->filename }}</a>
                                </div>
                            </div>
                        </li>
                    @endforeach
